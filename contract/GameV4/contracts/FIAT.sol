...
contract FIAT1 is ERC20, ERC20Burnable, Ownable {
  constructor(address initialOwner) ERC20('FIAT0', '$FIAT0') Ownable(initialOwner) {}
...
contract FIAT is ERC20, Ownable, ERC20Burnable {
  ...
  uint256 public maxTransactionAmount; // max amount can send/receive per txn
  uint256 public swapTokensAtAmount;
  uint256 public maxWallet; // max amount of fiat an address can hold

  bool public limitsInEffect = true; // token is unstable, remove after token is stable
  bool public tradingActive = false; // can trade
  bool public swapEnabled = false; // can swap
  ...
  mapping(address => bool) private _isExcludedFromFees; // addresses that is excluded from txn fees
  mapping(address => bool) public _isExcludedMaxTransactionAmount; // addresses that is excluded from maxTransactionAmount

  // store addresses that a automatic market maker pairs. Any transfer *to* these addresses
  // could be subject to a maximum transfer amount
  mapping(address => bool) public automatedMarketMakerPairs; // addresses can receive no limit amount
  ...
  constructor() ERC20('Fren Pet', 'Fren Pet') {
    IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x327Df1E6de05895d2ab08513aaDD9313Fe505d86); //base swap
