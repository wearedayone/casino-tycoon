...
interface IGangsterArena {
  ...
  function deposit(address to, uint256 tokenId, uint256 amount) external;

  function withdraw(address to, uint256 tokenId, uint256 amount) external;

  function burn(address[] memory to, uint256[] memory tokenId, uint256[] memory amount) external;
}
