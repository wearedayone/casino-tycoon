...
describe('POINT', function () {
  ...
  //     const prizeFee = await token.prizeFee();
  //     expect(prizeFee).to.equal(100);

  //     const liquidityFee = await token.liquidityFee();
  //     expect(liquidityFee).to.equal(50);

  //     const devFee = await token.devFee();
  //     expect(devFee).to.equal(30);
  ...
  describe('transactions', function () {
    ...
    it('swap eth to token no burn', async function () {
      ...
      const totalFees = await token.totalFees();
      const fees = (totalFees * amountOut[1]) / BASE_100_PECENT;
      ...
      const prizeFee = await token.prizeFee();
      const revShareTokens = (fees * prizeFee) / totalFees;
      const tokensForRevShare = await token.tokensForRevShare();
      ...
      const devFee = await token.devFee();
      const teamTokens = (fees * devFee) / totalFees;
      const tokensForTeam = await token.tokensForTeam();
    ...
    it('swap token to eth no burn', async function () {
      ...
      const totalFees = await token.totalFees();
      const fees = (totalFees * tokenAmount) / BASE_100_PECENT;
