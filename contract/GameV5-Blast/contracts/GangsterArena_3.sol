...
contract GangsterArena2 is Ownable, IGangsterArena {
  ...
  mapping(address => bool) private workerAddr;
  ...
  mapping(address => uint256) public addRep; // address -> number of additional reputation from gangwar
  ...
  ***************************
  ...
  function finalWarResult(
    ...
    require(workerAddr[msg.sender] == true, 'only worker');
    require(!gameClosed, 'Game is closed');
   ...
   */
  function burnNFT(address[] memory addr, uint256 tokenId, uint256[] memory amount) public {
    require(msg.sender == tx.origin, 'Reverting, Method can only be called directly by user.');
    require(workerAddr[msg.sender] == true, 'only worker');
    require(!gameClosed, 'Game is closed');
   ...
   */
  function setWinner(address[] memory to, uint256[] memory points) public {
    require(msg.sender == tx.origin, 'Reverting, Method can only be called directly by user.');
    require(workerAddr[msg.sender] == true, 'only worker');
   ...
   */
  function setGameClosed(bool value, uint256 _totalPoint) public {
    require(msg.sender == tx.origin, 'Reverting, Method can only be called directly by user.');
    require(workerAddr[msg.sender] == true, 'only worker');
    gameClosed = value;
  ...
  ***************************
  ...
  function devWithdraw() public {
    require(msg.sender == tx.origin, 'Reverting, Method can only be called directly by user.');
    require(msg.sender == devAddr, 'Reverting, Method can only be called directly by user.');
  ...
  function burn() public {
    require(msg.sender == tx.origin, 'Reverting, Method can only be called directly by user.');
    require(msg.sender == burnAddr, 'Reverting, Method can only be called directly by user.');
  ...
  ***************************
   ...
   */
  function setGameFee(uint256 _dev, uint256 _marketing, uint256 _prize, uint256 _retire) public onlyOwner {
   ...
   */
  function setReputation(uint256 _rpGangster, uint256 _rpGoon, uint256 _rpSHouse, uint256 _retireTax) public onlyOwner {
